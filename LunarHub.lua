...
spawn(function() 
    ...
    wait(1.5)
    require(game.ReplicatedStorage.Notification).new("<Color=Red>Sau vài ngày...<Color=/>"):Display()
    wait(.5)
    require(game.ReplicatedStorage.Notification).new("<Color=Red>Thì Lunar Hub đã được xuất bản ra.<Color=/>"):Display()
    wait(.14)
    ...
    wait(.29)
    require(game.ReplicatedStorage.Notification).new("<Color=Yellow> Channel Youtube : @dtqmodder_09 <Color=/>"):Display()
    wait(.36)
end)

local OrionLib = loadstring(game:HttpGet(('https://raw.githubusercontent.com/shlexware/Orion/main/source')))()
local Window = OrionLib:MakeWindow({Name = "Night Hub", HidePremium = false, IntroText = "LunarHub Library", SaveConfig = true, ConfigFolder = "Night Hub"})
...
local S = Window:MakeTab({
    Name = "📊 Stats",
    Icon = "rbxassetid://100247428606118",
    PremiumOnly = false
})
    ...
    while wait() do
        if _G.AutoEliteHunter then
            if game:GetService("Players").LocalPlayer.PlayerGui.Main.Quest.Visible == true then
                ...
                OrionLib:MakeNotification({
                    Name = "Lunar Hub(Noitification)",
                    Content = "[VN] : Làm ơn hủy nhiệm vụ farm llevel(nếu có)",
                    Image = "rbxassetid://88977940117252",
                    Time = 9
                })
            end
